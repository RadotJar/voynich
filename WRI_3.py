...
def main() :
    #input = get_input()
    directory = ["Species-Plantarum(I-III)", "Die-Epiphytische-Vegetation-Amerikas", "voynich-manuscript_Biological.txt_formatted"]
...
def results(fileName, directory, spacings1, spacings2, spacings3, WR_characters1 :dict, WR_characters2 :dict, WR_characters3 :dict) :
    ...
    #first plot
    plott.bar(characters1, WRC_val1, width=0.5)
    ...
    #second plot
    plott.bar(characters2, WRC_val2, width=0.5)
    plott.xlabel("Recurring Characters")
    plott.ylabel("Number of ocurrences")
    ...
    plott.plot(range(len(std_norm1)), std_norm1)
    plott.plot(range(len(std_norm2)), std_norm2)
    plott.plot(range(len(std_norm3)), std_norm3)
